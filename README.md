...
6. [🗺️ **State Sports**](./examples/06-state-sports/README.md) — state/province → teams → rosters across NFL/NBA/MLB/NHL: a live CountriesNow reference-data fetch, two chained whole-list `inc_parent` drills (league → team, team → roster), and a `conv_dict` that showcases all four converters (`link_to`/`inc`/`calc`/`pluck`) — a pure one-shot script, no Watershed.
