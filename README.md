...
6. [🗺️ **State Sports**](./examples/06-state-sports/README.md) — state/province → teams → rosters across NFL/NBA/MLB/NHL: a live CountriesNow reference-data fetch, whole-list + single-instance `inc_parent` drills, and this curriculum's first Tideweaver `Watershed` (a single-pass `CustomCurrent` → `Fjord` chain) driving the roster drill and board export.
