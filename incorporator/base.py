...
class Incorporator(BaseModel):
    ...
    def _build_instances(
        ...
        """
        if failed_sources:
            warnings.warn(f"Incorporator partial data returned: {len(failed_sources)} source(s) failed.", stacklevel=2)
        ...
        ActualClass = target_class or cast(
            Type[TIncorporator], schema_builder.infer_dynamic_schema("DynamicModel", transformed_data, cls),
        )
    ...
    async def test(
        ...
        try:
            ...
            from .methods.exceptions import IncorporatorNetworkError, IncorporatorFormatError
            ...
            if isinstance(e, IncorporatorNetworkError):
                ...
                if cause_name == "HTTPStatusError":
                    ...
                    print(
                        "   👉 Connection Timed Out: The server is unresponsive. Add `timeout=30.0` if it's just slow.")
                else:
    ...
    async def incorp(
        ...
        if inc_parent is not None:
            kwargs.update({
                ...
                "conv_dict": conv_dict, "name_chg": name_chg, "inc_page": inc_page,
            })
        ...
        if source:
            ...
            if infer_format(str(sample_source)) == FormatType.SQLITE and not kwargs.get("sql_query"):
                kwargs["sql_query"] = f"SELECT * FROM {kwargs.get('sql_table') or cls.__name__.lower()}"
        ...
        parsed_data, failed_sources = await network.fetch_concurrent_payloads(
            source_list=source_list, is_file_mode=is_file_mode, inc_page=inc_page,
            payload_list=kwargs.pop("payload_list", None), **kwargs,
        )
    ...
    async def refresh(
        ...
        parsed_data, failed_sources = await network.fetch_concurrent_payloads(
            ...
            inc_page=inc_page, payload_list=payload_list, **kwargs
        )
