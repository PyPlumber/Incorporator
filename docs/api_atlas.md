...
5. Verb-typed `Current` subclasses dispatch differently: `Stream` runs chunking `cls.stream(...)` and parks a strong-ref snapshot on `_tideweaver_snapshot`; when `Stream.parent_current` is set, it instead filters the parent's snapshot via `parent_filter` and calls `cls.incorp(inc_parent=filtered, ...)` directly (parent-child drill mode); `Fjord` is a per-tick flush (`outflow(state)` → build → export); `Export` runs `cls.export(...)`.
...
| `Stream.parent_current` / `Stream.parent_filter` | `Stream` fields | parent-child drill | `parent_current: str` names an upstream `Stream` current in the same watershed; `parent_filter` is a `Callable[[row], bool]` or `(attr, op, value)` tuple that subsets the parent's `_tideweaver_snapshot` before the drill. When set, `_tick_stream` calls `cls.incorp(inc_parent=filtered, ...)` instead of `cls.stream()`. Watershed auto-derives a hard-gate edge from `parent_current` to this stream. |
