...
3. Run cross-source analysis on the captured profiles, first partitioning them into healthy vs. failed (a probe that raised, or came back with no signal, is marked and excluded from shape inference — see **Probe-failure escalation** below): detect `fanout` (one source's PK appears as a non-PK field in all others), `diamond` (multiple sources share a PK field name — needs a tail Fjord), `parallel` (disjoint field sets), or `custom` (some overlap but no clear pattern), computed over the healthy sources only.
...
5. Dispatch on `output=`: `"report"` prints inspector output + cross-source hints (plus a dedicated `FAILED SOURCES` block when any probe failed); `"python"` emits a paste-ready Python module (class defs + `Stream(...)` constructors + `Watershed.<shape>(...)` + a `Tideweaver` runner, with a `# WARNING:` header comment when any probe failed); `"json"` emits a paste-ready `watershed.json` that round-trips through `load_watershed()` (plus an advisory `_probe_warnings` key when any probe failed); `"plan"` returns the structured `OrchestrationPlan` dataclass directly (including `failed_sources`) so callers can probe → tune → run in one expression via `plan.to_watershed()`.

**Probe-failure escalation** — a failed probe (raised exception, or a fetch that returned no signal) is never silently folded into shape inference as if it were a legitimate disjoint source. Partial failure: the failed source(s) are excluded from the fanout/diamond/parallel/custom heuristic, surfaced via `OrchestrationPlan.failed_sources` (name → error string) and `notes`, and visible in all three rendered outputs. Total failure (every source's probe fails): there is no signal left to infer a shape from, so `architect()` raises `IncorporatorError` rather than return a fabricated plan — this applies uniformly across all four `output=` modes.
...
- Raises `IncorporatorError` when **every** source's probe fails (no signal left to infer a shape from), regardless of `output=`.  When only *some* probes fail, the healthy sources are still resolved normally and the failure surfaces via `OrchestrationPlan.failed_sources`.
...
- `inc_code=` — field name on each record that becomes the primary key in `inc_dict`. Pass the field name (e.g. `"id"`); the framework reads each record's value at that key. **Duplicate keys upsert:** when two records in one batch carry the same `inc_code`, the later record wins — `inc_dict` is a plain dict keyed by `inc_code`, so a duplicate is treated as an update to the existing record, not an error. This is the same overwrite `refresh()` relies on to publish each source's latest tick under a stable key.
...
**Why build-time joins survive refresh.** `link_to(dataset, ...)` is lazy and live by design: it stores a reference to `dataset` and re-reads `dataset.inc_dict` on **every** lookup call rather than copying it once at construction. A `link_to(PeerClass)` built before `PeerClass` has ever ticked (e.g. an `inflow(state)` conv_dict entry constructed at seed time, or a JSON-config `"link_to(PeerClass)"` token resolved before the peer source runs) simply resolves to `None` until the peer populates, then starts resolving correctly from that point on — it is never permanently stuck empty. `Incorporator.refresh()` mutates existing instances' *fields* in place rather than replacing the objects, so a resolved reference from an earlier build-time join keeps seeing current data across refresh waves for free. This shares the same `ClassVar[WeakValueDictionary]` liveness contract as reading `Cls.inc_dict` directly — for an `IncorporatorList` target, the closure's own strong reference to the list keeps rows alive; for a bare class target, liveness depends on another strong-ref holder elsewhere in the pipeline, the same as any other `inc_dict` read.  **Key typing is one-way:** a lookup tries the foreign-key value as-is, then falls back to `str(key)` — so an integer FK resolves against a string-keyed registry, but a string FK does **not** resolve against an integer-keyed registry (there is no reverse `int(key)` bridge, by design).  Coerce both sides to the same type in your `conv_dict` (e.g. `inc(str, "id")` on the target's `inc_code` field and on the FK) so the join is symmetric.
...
Both patterns are worked through in [Tutorial 9 — NASCAR Fantasy Fjord](../examples/09-nascar-fantasy-fjord/README.md)'s `FantasyTeam` view. T9's `Race` FK joins (`track_id` → `Track`, `pole_winner_driver_id`/`winner_driver_id` → `Driver`) and [Tutorial 10 — Multi-Source Fjord](../examples/10-multi-source-fjord/README.md)'s `CoinGecko` ↔ `BinancePair` spread resolve the same way — read-time in `outflow(state)` via `state["Peer"].inc_dict.get(key)`. Both tutorials use the single read-time join idiom throughout, with no build-time `link_to` / `inflow(state)`: a flat FK *can* be resolved at build time, but read-time in `outflow(state)` is the default the framework's own fjord example uses (see `link_to()` above for when a build-time join is genuinely required).

**No read-time registry accessor is being added.** A tempting "fix" for read-time joins is a convenience method like `inc_get(key)` or `inc_lookup(key)` that wraps `Cls.inc_dict.get(key)` with friendlier syntax. This is deliberately **not** a framework primitive — `link_to()` already *is* that accessor (see above), and for the two honest read-time boundaries (list-fan-out, runtime dataset choice) it can't be used directly because they need per-element or per-row dataset selection, not a single static target. Those two cases are genuinely better served by keeping the lookup inline in `outflow(state)`, reading from the `state` snapshot's own `IncorporatorList.inc_dict` (a `@property` proxy onto the class-level registry, safe because the whole snapshot was captured together) — not by adding a new primitive that would encourage bypassing `state` entirely. (The GC-race / stale-read risk of reading a bare `Cls.inc_dict` directly inside `outflow(state)`, instead of the `state` snapshot's `IncorporatorList.inc_dict`, is a separate concern from `link_to()`'s own liveness contract described above — it applies to *raw* class-level registry access, not to `link_to()` calls.)
