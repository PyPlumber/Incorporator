...
| **T6** | [`06-state-sports/`](./06-state-sports/) | CountriesNow (2) + ESPN (~143) | ~10-30 s + ~15 s window | 🗺️ State/province → teams → rosters: a live CountriesNow reference-data fetch, whole-list + single-instance `inc_parent` drills, and this curriculum's first Tideweaver `Watershed` (`CustomCurrent` → `Fjord`, single-pass). |
