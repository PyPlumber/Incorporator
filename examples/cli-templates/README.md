...
| [`outflow_example.py`](./outflow_example.py) | Canonical `outflow(state)` skeleton — paired with `fjord-basic.json`, referenced by the [crypto-graph-mapping](../appendix/crypto-graph-mapping/) and [xml-post-audit](../appendix/xml-post-audit/) appendices. |
