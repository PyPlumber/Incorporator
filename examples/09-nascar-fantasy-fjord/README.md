...
T8 introduced streaming polling on single-source registries.  **Real-world multi-source pipelines** look like fantasy-sports league scoring: three concurrent live feeds (Cup, Xfinity, Truck series), two master-data joins (driver roster, track schedule), one fused weekly-points output.  This tutorial walks that real-world fjord shape — and T10 will introduce `fjord()` formally on the minimum-viable two-source case.  You're getting the production shape first, the abstraction next.

The minimum viable fjord is two co-equal sources with one outflow and one export file — that's what T10 shows on crypto.  Real production joins are messier.  You have **dependent sources** (one source's foreign keys reference another), **mixed API + local-file inputs** (market data on the wire, business config in version control), **multiple analytical views** from the same fused state, and **sentinel values** in raw APIs that need filtering at the graph boundary.
...
[`examples/09-nascar-fantasy-fjord/nascar_fantasy.py`](../../examples/09-nascar-fantasy-fjord/nascar_fantasy.py)
and [`examples/09-nascar-fantasy-fjord/driver.py`](../../examples/09-nascar-fantasy-fjord/driver.py).
...
examples/09-nascar-fantasy-fjord/fixtures/league_teams.json                       →  LeagueRoster   (8 rows)
```
...
`examples/09-nascar-fantasy-fjord/nascar_fantasy.py` defines the six source
...
def outflow(state: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    # (full code in examples/09-nascar-fantasy-fjord/nascar_fantasy.py)
...
`examples/09-nascar-fantasy-fjord/driver.py` calls fjord with the six sources
...
#    container path:
#         "inc_file": "config/league_teams.json"
#    NOT  "inc_file": "examples/09-nascar-fantasy-fjord/fixtures/league_teams.json"
