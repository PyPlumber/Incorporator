...
The output class is **derived dynamically** from the ``code_file`` filename
...
async def test_fjord_derives_class_name_from_stem(
    ...
    """code_file ``crypto_spread.py`` → dynamic class is named ``CryptoSpread``."""
    monkeypatch.setattr(fetch, "execute_request", mock_execute_request)
    monkeypatch.chdir(tmp_path)
...
async def test_fjord_outflow_file_must_define_outflow(tmp_path: Path) -> None:
    """code_file without a top-level outflow() function raises clearly."""
...
@pytest.mark.asyncio
async def test_fjord_code_file_missing_raises(tmp_path: Path) -> None:
